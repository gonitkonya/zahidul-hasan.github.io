...
Obviously, <center>$e^{\eta W_{n,i}} \leq \sum_{j = 1}^{k} e^{\eta W_{n, j}} = T_{n} = T_{0} \prod_{t = 1}^{n} \frac{T_{t}}{T_{t-1}} = k \prod_{t = 1}^{n} \frac{T_{t}}{T_{t-1}}$</center>    
Since, <center>$W_{t,i} = W_{t-1,i} + \bar X_{t,i}$,   </center>
So, <center>$e^{W_{t,i}} = e^{W_{t-1,i}}  e^{\bar X_{t,i}}$.    </center> 
Then we get, <center>$\frac{T_{t}}{T_{t-1}} = \sum_{j = 1}^{k} \frac{e^{W_{t-1,i}}}{T_{t-1}}  e^{\bar X_{t,i}} = \sum_{j = 1}^{k} p_{t,j} e^{\bar X_{t,i}} $     
