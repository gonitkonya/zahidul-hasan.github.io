...
Suppose, we are talking about ad recommendation. We have $k$ ads to render to a user. For each ad, we keep track of $(\alpha, \beta)$. We start with $(\alpha = 0, \beta = 0)$. An implementation of Thompson Sampling in this scenario is shown below:
