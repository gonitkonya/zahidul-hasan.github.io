...
Now, let's express them in their reduced forms. If there is a fraction such as, $\frac{a}{d}$, then definitely $a \leq d$ and $\gcd(a,d) = 1$. Because, otherwise, it wouldn't be an irreducible fraction. And there will be exactly $\varphi(d)$ such $a$ for each $d$. And in total, there are $n$ fractions. QED.
