...
<br>
<strong>Periodicity:</strong> A Markov Chain is periodic if there exists a $t > 1$ for which there exists a state $s$ so that $s$ can only be visited at time steps $t, 2t, 3t, \cdots $. A Markov Chain is aperiodic if every state can be visited at all time steps.      
