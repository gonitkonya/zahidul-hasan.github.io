...
<strong>Stochastic Matrix:</strong> A stochastic matrix is a square matrix where all of the entries are non-negative and the sum of each column is 1. Every stochastic matrix represents a finite markov chain. For example, the above state transition diagram can be expressed as a stochastic matrix like this:    
$\begin{bmatrix}
1 & 2 & 3\\
a & b & c\\
\end{bmatrix}$      
